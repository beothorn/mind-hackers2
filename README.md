...
A game/experiment using the OpenAI Responses API. The browser client now uses `gpt-5.4-mini`, a current cost-efficient GPT model recommended for lower-cost/low-latency workloads while still supporting text generation through the Responses API.
...
Open the local dev-server URL, enter an OpenAI API key, and play the restaurant simulation. The key is stored only in browser local storage and requests are sent directly from the browser to OpenAI.

## Model and API

- Default model: `gpt-5.4-mini`.
- API endpoint: `POST https://api.openai.com/v1/responses`.
- Token check: `GET https://api.openai.com/v1/models/gpt-5.4-mini`.
- Short yes/no state-transition prompts use low reasoning effort and small output budgets to keep costs down.

# Core idea

If we use GPT model to decide state transition, would the results be consistent? Would it be a fun game?
We can constraint the player to a list of predetermined actions to have control over the narrative.
Then after every interaction we ask GPT model very specific questions to update the game state.
The game state is hidden from the player. Only the gpt narrative of the events is shown.

The steps would be:
- Generate a text based on state
- Ask GPT model if a state should be changed
- Update game state
- Describe situation and state change, ask GPT model to narrate this (GPT model is treated like a renderer for the game state)
...
// funny how you need to convince GPT model to improve the text
...
If we have a game like "The sims", describe in a text the log of interactions, the player statuses, the social status and then ask GPT model to decide what to do next, what would happen?
...
For example, given the player status and setting we can generate this and ask GPT model for more input for the game:
```
...
Another idea is to allow the player to add one phrase to this premise to try hacking the output from GPT model by doing an idea inception on the npcs.
...
GPT has a really hard time understanding who said what.
...
It would be ok if it was just creative, but usually it drops whatever the text is on about very abruptly and talks about something else.

Sometimes it also likes to just close the story. Even if the text says that something should not happen right now (like paying the bill), GPT model just
